...
(function () {
  ...
  function showError(msg) {
    ...
    errorBox._t = setTimeout(() => { errorBox.textContent = ''; }, 2500);
  }
  ...
  function buildThumbs(imageUrls) {
    if (!thumbsCol) return;
    thumbsCol.innerHTML = imageUrls.map((url, i) => `
      <button type="button"
  ...
  function bindThumbs() {
    thumbsCol.querySelectorAll('.pd-thumb').forEach(btn => {
      btn.addEventListener('click', () => {
        thumbsCol.querySelectorAll('.pd-thumb').forEach(b => b.classList.remove('pd-thumb--active'));
  ...
  function syncQty() {
    ...
    if (isNaN(v) || v < 1) v = 1;
    if (v > MAX_QTY) { v = MAX_QTY; showError(`Max ${MAX_QTY} per order`); }

    const stock = getSelectedVariant()?.stock ?? 0;
    if (v > stock) { v = Math.max(1, stock); showError(`Only ${stock} in stock`); }
  ...
  document.querySelectorAll('.pd-color').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.pd-color').forEach(b => b.classList.remove('pd-color--active'));
  ...
  function buildSizes(colorVariants) {
    if (!sizesWrap) return;
    sizesWrap.innerHTML = colorVariants.map(v => `
      <button type="button"
  ...
  function bindSizes() {
    sizesWrap.querySelectorAll('.pd-size').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        sizesWrap.querySelectorAll('.pd-size').forEach(b => b.classList.remove('pd-size--active'));
  ...
  document.querySelectorAll('.pd-related-wish').forEach(btn => {
    btn.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      // POST via fetch or redirect — wired up per your wishlist route
